...
def parse_args():
    ...
    parser.add_argument('--output', type=str, default="/home/jupyter-lpy/project/target_detection/CAD_VQA/cc-3.7-sonnet/output/p5_result/",
                        help='Path to save the results')
...
第四步回答问题：基于建筑元素识别内容、文本元素识别内容以及建筑工程领域知识，按照问题类型以及回答要求给出答案及分析推理过程。
...
(3)面积问题
   最终答案：数值+平方米，如 `20平方米` 
示例：{问题: CAD图像中位于右上角的电梯的实际面积是多少？ 最终答案：4平方米}
(4)尺寸问题
最终答案：数值+单位*数值+单位，如‘多少m*多少m’
示例：{问题: CAD图像中位于左下角的浴室的实际尺寸多大？最终答案：0.9m*0.9m}
(5)询问载重参数问题
最终答案：数值+kg，如`1000kg`
示例：{问题: CAD图像中位于左上角的电梯的载重参数是多少？最终答案：1000kg}
(6)询问“符合性”问题
   最终答案：只回答`符合`或`不符合`。
...
(9) 询问规格参数问题
最终答案：直接给出规格参数
示例：{问题: CAD图像中位于右上角的窗规格参数是多少？最终答案：FM1022 }
