...
def parse_args():
    ...
    parser.add_argument('--output', type=str, default="/home/jupyter-lpy/project/target_detection/CAD_VQA/QWen/output/qwen_result5/",
                        help='Path to save the results')
...
**分析推理过程提示**
第一步建筑元素识别：识别出图像中的建筑元素的类型、相对位置、相对方位。
第二步文本元素识别：识别建筑平面CAD图像中的文本元素内容及相对位置。
第三步判别问题类型：理解问题内容，归类到系统提示中的九类问题。
第四步回答问题：基于建筑元素识别内容、文本元素识别内容以及建筑工程领域知识，按照问题类型以及问题答案要求给出分析推理过程及问题答案
**系统提示**
...
(9) 询问规格参数问题
问题答案要求：直接给出规格参数，如“FM1022”
示例：{问题: CAD图像中靠近左上角的窗规格参数是多少？答案：FM1022 }

**返回要求**：
1.最终返回内容为：分析推理过程：xxx----问题答案：xxx
2.问题答案严格按照问题类型的问题答案要求进行回答，问题答案中禁止添加标点及其他无关上下文（如：‘答案是’）
3. 如果不知道问题答案，直接返回无法回答，请勿按照示例答案给出
    """
    # 遍历文件夹
    image_id_list = []
